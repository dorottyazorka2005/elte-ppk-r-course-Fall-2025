...
AIC(simple_model, modified_complex_model)      #Checking AIC and BIC values
...
LRstat; pvalue_LR
```
A hierarchical regression was conducted to examine predictors of pain. In the simpler model, demographic predictors (age and sex) accounted for 1.6% of the variance in pain (R² = .016). Adding psychological and hormonal predictors (STAI_trait, pain_cat, mindfulness, and cortisol_serum) to the model, the final model accounted for 7.5% of the variance (F(4, 153) = 2.46, p = .048, R² = .075). The AIC value was lower for the more complex model (AIC = 889.99), indicating a better fit. A likelihood ratio test also showed that adding these predictors significantly improved the prediction of pain (χ²(4) = 9.96, p = .041).