...
```{r}
pain <- read_excel("assignment_3_dataset.xlsx")
print(pain)
```
...
```{r}
str(pain) #Get the structure of variables

pain %>% 
  summarise(across(everything(), ~ sum(is.na(.)))) #There are no missing values

pain %>%                                            #Summary statistics for numeric variables
...
ggplot(pain, aes(age)) + geom_histogram() #Checking for distribution and outliers
ggplot(pain, aes(sex)) + geom_bar() #There is answer "woman" instead of "female"
ggplot(pain, aes(STAI_trait)) + geom_histogram() 
ggplot(pain, aes(pain_cat)) + geom_histogram() 
ggplot(pain, aes(mindfulness)) + geom_histogram() 
ggplot(pain, aes(cortisol_serum)) + geom_histogram() 
ggplot(pain, aes(cortisol_saliva)) + geom_histogram() #My creative solution:)))

ggplot(pain, aes(x = age, y = pain)) +                         #Checking the relationship between variables
  geom_smooth(method = "lm", se = FALSE, color = "red")
ggplot(pain, aes(x = sex, y = pain, fill = sex)) +
  geom_boxplot() + ylim(0, 20)

pain %>%                                           #Group statistics
  group_by(sex) %>%
  summarise(
...
numeric_var <- pain %>% select(where(is.numeric))
